...
import UIKit
 ...
 protocol LiquidSwipeContainerDataSource {
 ...
 protocol LiquidSwipeContainerDelegate {
 ...
 class LiquidSwipeContainerController: UIViewController {
    ...
    public var btnNext: UIButton = {
        ...
        button.setImage(UIImage(named: "btnNext.png"), for: .normal)
        return button
    }()
    
    private var initialHorRadius: CGFloat = 90.0
    private var maxHorRadius: CGFloat {
        return view.bounds.width * 0.8
    }
    
    private var initialVertRadius: CGFloat = 172.0
    private var maxVertRadius: CGFloat {
        return view.bounds.height * 0.9
    }
    private var initialSideWidth: CGFloat {
        if #available(iOS 11.0, *) {
            return 6.0 + view.safeAreaInsets.right
        }
        return 6.0
    }
    private var initialWaveCenter: CGFloat  {
        return view.bounds.height / 2 // * 0.7167487685
    }
    ...
    private func configureBtnNext() {
        ...
        csBtnNextLeading = btnNext.leadingAnchor.constraint(equalTo: view.trailingAnchor, constant:  -(initialHorRadius + initialSideWidth) + 28.0)
    ...
    private func animate(view: UIView, forProgress progress: CGFloat, waveCenterY: CGFloat? = nil) {
        ...
        csBtnNextLeading?.constant = -(mask.waveHorRadius + mask.sideWidth - 28.0)
        mask.updatePath()
        ...
        if sender.state == .began {
            ...
            let currentViewAnimation = POPCustomAnimation {[weak sender] (target, animation) -> Bool in
                ...
                if !self.shouldCancel {
                    ...
                    self.csBtnNextLeading?.constant = -(mask.waveHorRadius + mask.sideWidth - 28.0)
    ...
    private func showNextPage() {
       ...
       if(isDarkMode)
      {
       currentViewController!.view.backgroundColor = Color.BlackColor
       }
      if(currentViewController?.view.tag == 4)
       {
          btnNext.setImage(UIImage(named: "btnNextGray.png"), for: .normal)
       }else
      {
         btnNext.setImage(UIImage(named: "btnNext.png"), for: .normal)
      }
        currentPageIndex += 1
   ...
   var isDarkMode: Bool {
    ...
    private func showPreviousPage() {
        ...
        currentViewController = previousViewController
         if(nextViewController?.view.tag == 4)
            {
               btnNext.setImage(UIImage(named: "btnNextGray.png"), for: .normal)
            }else
           {
    ...
    override open func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: { (_) in
            ...
            self.csBtnNextLeading?.constant = -(self.initialHorRadius + self.initialSideWidth - 28.0)
