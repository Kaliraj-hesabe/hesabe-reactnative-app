...
//  HesabeMerchant
//
//  Created by pramod yadav on 03/06/21.
//
            ...
            _ = getAppInfo { (info, error) in
                if let appStoreAppVersion = info?.version {
                    if let error = error {
                        ...
                        if(appStoreAppVersion.compare(currentVersion, options: .numeric) == .orderedAscending)
                        {
                          let cv = currentVersion.split(separator: ".")
                          let av = appStoreAppVersion.split(separator: ".")
                        
                          if(Int(cv[0])! > Int(av[0])!)
                          {
                             
                          }
                         else
                          {
                            print("AppStore Version: \(av[0]) > Current version: ",currentVersion)
                            if(Int(cv[0])! == Int(av[0])!)
                            {
                               if(Int(av[1])! > Int(cv[1])! || Int(av[2])! > Int(cv[2])!)
                               {
                            ...
                            else
                            {
                              DispatchQueue.main.async {
                                  let topController: UIViewController = (UIApplication.shared.windows.first?.rootViewController)!
                                  topController.showAppUpdateAlert(Version: (info?.version)!, Force: true, AppURL: (info?.trackViewUrl)!)
                              }
                            }
    ...
    private func getAppInfo(completion: @escaping (AppInfo?, Error?) -> Void) -> URLSessionDataTask? {
      ...
      // Você deve prestar atenção em que país o app está disponível, no meu caso eu coloquei Brasil */br/*
        ...
        guard let identifier = self.getBundle(key: "CFBundleIdentifier"),
              let url = URL(string: "http://itunes.apple.com/kw/lookup?bundleId=\(identifier)") else {
                DispatchQueue.main.async {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
                ...
                do {
                  ...
                  //print(result.results.first?.version)
...
extension UIViewController {
    @objc fileprivate func showAppUpdateAlert( Version : String, Force: Bool, AppURL: String) {
      ...
      if let str = UserDefaults.standard.string(forKey: "Language") {
        ...
        let alertTitle = "New version".localized(templang)
      let alertMessage = String(format:"A new version of %@ is available on AppStore. Update now!".localized(templang),appName)

        let alertController = UIAlertController(title: alertTitle, message: alertMessage, preferredStyle: .alert)

        if !Force {
            let notNowButton = UIAlertAction(title: "Not now".localized(templang), style: .default)
            alertController.addAction(notNowButton)
        }

        let updateButton = UIAlertAction(title: "Update".localized(templang), style: .default) { (action:UIAlertAction) in
